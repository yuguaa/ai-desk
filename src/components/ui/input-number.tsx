...
function InputNumber({ className, value, min, max, step = 1, isValid, onValueChange, onBlur, onKeyDown, ...props }: InputNumberProps) {
  ...
  const submit = (parsed: number) => {
    const nextValue = normalize(parsed);
    if (!Number.isFinite(parsed) || !Number.isFinite(nextValue) || (isValid && !isValid(nextValue))) {
  ...
  const nudge = (direction: -1 | 1) => submit(value + direction * step);
