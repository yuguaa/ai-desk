...
export function TimelineItemView({ item }: { item: TimelineItem }) {
  if (item.type === "user") {
    return <Message from="user" className="py-4 pt-6"><div className="flex max-w-[80%] flex-col items-end gap-1.5"><MessageContent className="rounded-[var(--radius-md)] border border-[var(--border-subtle)] bg-[var(--bg-surface-raised)] px-3 py-2 text-[var(--font-size-13)] leading-5 text-[var(--text-primary)]"><MessageResponse>{item.text}</MessageResponse></MessageContent><div data-slot="user-message-meta" className="flex items-center justify-end gap-1.5"><MessageCopyButton text={item.text} /><div className="font-mono text-[var(--font-size-9-5)] text-[var(--text-tertiary)]">{item.time}</div></div></div></Message>;
  }
