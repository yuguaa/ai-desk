...
const images: ImageAttachment[] = [{ id: "img1", name: "示例.png", type: "image", data: "aGVsbG8=", mimeType: "image/png" }];
...
function render(onRemove?: (id: string) => void) {
  ...
  return act(() => root.render(<ImageAttachments images={images} onRemove={onRemove} />));
}

describe("ImageAttachments", () => {
  it("显示缩略图与名称，移除传回附件 ID", async () => {
    ...
    await act(() => container.querySelector<HTMLButtonElement>('[aria-label="移除图片：示例.png"]')!.click());
  ...
  it("历史附件不显示移除，Radix 预览可关闭并恢复焦点", async () => {
    await render();
    expect(container.querySelector('[aria-label="移除图片：示例.png"]')).toBeNull();
