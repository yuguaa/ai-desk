...
export function Reasoning({ content, status = "completed", defaultOpen = true }: { content: string; status?: "running" | "completed"; defaultOpen?: boolean }) {
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="my-1 w-full overflow-hidden rounded-[var(--radius-sm)] border border-[var(--border-subtle)] bg-[var(--bg-surface-raised)] text-[var(--font-size-12)] text-[var(--text-secondary)]">
      ...
      <CollapsibleContent><div data-slot="reasoning-content" className="flex max-h-64 flex-col-reverse overflow-y-auto"><div className="w-full shrink-0 whitespace-pre-wrap border-t border-[var(--border-subtle)] p-2.5 text-[var(--font-size-12)] leading-5 text-[var(--text-secondary)]">{content}</div></div></CollapsibleContent>
    </Collapsible>
  );
}
