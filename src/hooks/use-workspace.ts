  ...
  function executeConversationTurn(conversation: ConversationRecord, text: string, beforeRun: TurnPreparation | undefined, commandType: "prompt" | "steer", attachments: Attachment[] = []) {
    ...
    const execution = (beforeRun ? beforeRun(submittedTurn) : Promise.resolve())
      ...
      .then(() => {
        ...
        /* 排队后模型可能变化，发送前重新检查该会话实际使用的模型。 */
        return sendRpcCommand(conversation.id, { type: "get_state" }).then((response) => {
          const data = response?.data as { model?: { input?: string[] } } | undefined;
          if (!data?.model?.input?.includes("image")) throw new Error("当前模型不支持图片，请选择支持图片的模型后重新发送");
        });
      })
      .then(() => {
        ensureExecutionActive();
