...
describe("useWorkspace", () => {
  it.each(["文字", "图片", "读取中"])("有未发%s时阻止队列编辑且不覆盖草稿", async (kind) => {
  ...
  it.each(["resolve", "reject"])("取消含图任务后迟到的 %s 不复活任务或清除新任务状态", async (outcome) => {
    await mountWorkspace();
    ...
    act(() => workspace!.sendMessage(() => new Promise<void>(() => undefined)));
    const activeTurn = workspace!.activeTurnIndexes.c1;
    expect(activeTurn).toBeDefined();
    runtime.sendPiCommand.mockClear();
    await act(async () => {
      if (outcome === "resolve") resolve();
      else reject(new Error("迟到的准备失败"));
    });
    expect(workspace!.queuedTurns).toEqual([]);
    expect(workspace!.activeTurnIndexes.c1).toBe(activeTurn);
    expect(hasSentCommand("prompt")).toBe(false);
    expect(workspace!.timeline.some((item) => item.type === "assistant" && item.text.includes("迟到的准备失败"))).toBe(false);
  });
  ...
  it("发送失败回到原会话队列，不覆盖新草稿；切换重载后仍可编辑重试", async () => {
    ...
    expect(findLastCommand("prompt").command).toMatchObject({ images: [{ type: "image", data: attachedImage.data, mimeType: attachedImage.mimeType }] });
  });
  ...
  it("队列编辑保留图片，出队前重新检查模型能力", async () => {
    ...
    const state = findLastCommand("get_state").command;
    await act(async () => { runtime.onEvent?.({ conversationId: "c1", event: { type: "response", id: state.id, command: "get_state", success: true, data: { model: { ...visionModel, input: ["text"] } } } }); });
    expect(hasSentCommand("prompt")).toBe(false);
